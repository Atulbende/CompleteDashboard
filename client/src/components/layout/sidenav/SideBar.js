...
export default function SideBar({i,Lable,Link,icon,Access,IsClickable,subMenu}) {
    const [toggle,SetToggle]=useState(false);
    const Userrole=[1,2,3,4];
        ...
        return (
            <div  className={toggle ? 'menu-item open':'menu-item'} key={i} ><div onClick={()=>{SetToggle(!toggle)}} className={`sidebar-item`}><span className='icon'> <i  className={icon}></i></span><span className='sidebar-title'> {Lable}</span><i className={toggle ? 'fas fa-chevron-down':'fas fa-chevron-right'}  ></i></div>
                 <div className={`child`}>   {
                        ...
                        return ( <SideBar key={index} i={index} Lable={val.title} Link={val.link} Access={val.access}  icon={val.icon} IsClickable={val.IsClickable} subMenu={val.subMenu}></SideBar>)
                        })
                    } 
                    </div>       
            </div>)
